...
from data.texts import balance_answer, buy_no_email_answer, check_email_answer, choose_subscription_type_answer, \
...
async def command_profile(message: types.Message):
    try:
        ...
        await message.answer(balance_answer.format(user_name=user.firstname, user_subscription=user_subscription), reply_markup=ikb_buy)
